	...
	if ((task->b > 0) || (task->c > 0)) {
		...
		if (best_star == task->dest) {	/*stay put */
			if (best_planet != nil) {
				...
				} else if ((best_planet->team == ENEMY) && (best_planet->conquered)) {	/*decide whether to split */
