...
//char field;
{
	...
	case 'V':
		...
		if (vel[player] < max_vel)
			printw(" res: %3d need:%4d", vel_working[player], vel_req[vel[player] + 1]);
		else
			printw("                   ");
		break;
	case 'R':
		...
		if (range[player] < bdsize)
			printw(" res: %3d need:%4d", ran_working[player], ran_req[range[player] + 1]);
		else
			printw("                   ");
		break;
	case 'W':
		...
		if (weapons[player] < 10)
			printw(" res: %3d need:%4d", weap_working[player], weap_req[weapons[player] + 1]);
		else
			printw("                   ");
		break;
	};			/*switch ( */
