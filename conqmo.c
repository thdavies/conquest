	...
	do {
		for (i = 1; i <= 26; i++) {
			if ((tf[tm][i].dest != 0) && (tf[tm][i].eta != 0)) {
				...
				if ((!stars[tf[tm][i].dest].visit[tm]) && (tf[tm][i].eta == 0) && (tm == player)) {
					...
					if (tf[tm][i].s != 0)
						prob = (s_e_prob + rnd(s_e_var) * tf[tm][i].s) / 100.0;
					if (tf[tm][i].c != 0)
						prob = (c_e_prob + rnd(c_e_var) * tf[tm][i].c) / 100.0;
					if (tf[tm][i].b != 0)
						prob = (b_e_prob + rnd(b_e_var) * tf[tm][i].b) / 100.0;
					if (prob > 100)
						prob = 100;
					loss = true;
				...
				if (tf[tm][i].dest != 0) {
					if (tm == player) {
						...
						if ((tf[tm][i].eta == 0)) {
							...
							if (!stars[tf[tm][i].dest].visit[tm]) {
								...
								update_board(tf[tm][i].x, tf[tm][i].y, left);
					...
					if ((tm == ENEMY) && (tf[tm][i].eta == 0)) {
						...
						if (tf_stars[tf[tm][i].dest][ENEMY] > 0) {
							there = 1;
							while ((there == i) || (tf[ENEMY][there].dest != tf[ENEMY][i]
										.dest) || (tf[ENEMY][there].eta != 0))
								there = there + 1;
